...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: "Ekoloji Hesabatlar", href: "#impact" },
    { name: "Təşəbbüslər", href: "#initiatives" },
  ];
      ...
      <div className="container max-w-7xl mx-auto px-4 lg:px-8 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-3 group">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs font-semibold text-primary tracking-wide">Dayanıqlı İnkişaf</span>
          </div>
        </Link>

        {/* Desktop Navigation */}
        <nav className="hidden md:flex items-center gap-8">
          <div className="flex items-center gap-6 bg-slate-100/50 dark:bg-slate-800/50 px-6 py-2.5 rounded-full backdrop-blur-sm border border-slate-200 dark:border-slate-700">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className="text-sm font-medium text-slate-600 hover:text-primary dark:text-slate-300 dark:hover:text-primary-light transition-colors"
              >
                {link.name}
              </Link>
            ))}
